#ifndef ARBRENARI_HPP
#define ARBRENARI_HPP
...
public:
  ...
  T maximo(){
      int x=primer_node->info;
      maxim(primer_node,x);
  }
  void maxim(node_arbreNari* n,int &x){
      if (n!=NULL){
          if (n->info>x) x=n->info;
          for (int i=0;i<N;++i){
              maxim(n->seg[i],x);
          }
      }
  }
  void arb_sumas(ArbreNari& asum){
      node_arbreNari* n;
      int x=0;
      suma(primer_node,n,x);
      asum.primer_node=n;
  }
  void suma(node_arbreNari* a,node_arbreNari* & n, int& x){
      if (a== NULL){
          x=0;
      }
      else{
          for (int i=0; i<N;++i){
              node_arbreNari* fill;
              int x1;
              suma(a->seg[i],fill,x1);
              cout << "x1: "<<x1<<" en: " << a->info << endl;
              n=new node_arbreNari;
  ...
  // Extensiones, poner o quitar comentario para usarlas
